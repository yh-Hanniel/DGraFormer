...
class DGraFormer_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
                 ...
                 pe: str = 'zeros', learn_pe: bool = True, fc_dropout: float = 0., head_dropout=0, revin=True,
        ...
        # Backbone
        self.mtt = MTT(c_in, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len,
        ...
        self.n_vars = c_in
        self.d_gcn = d_gcn
        self.pred = Predictor(self.n_vars, self.head_nf, target_window, head_dropout=head_dropout)
    ...
    def forward(self, z, time_index, current_epoch):  # z: [bs x nvars x seq_len] # time_index: [bs x timeinfo]
        ...
        # model
        z = self.mtt(z)  # z: [bs x nvars x d_model x patch_num]
...
class MTT(nn.Module):
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        # Encoder
        self.encoder1 = TransformerEncoder(q_len, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                           attn_dropout=attn_dropout, dropout=dropout,
                                           pre_norm=pre_norm, activation=act, res_attention=res_attention,
                                           n_layers=n_layers,
                                           store_attn=store_attn)
        self.encoder2 = TransformerEncoder(q_len / 2, d_model * 2, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                           attn_dropout=attn_dropout, dropout=dropout,
                                           pre_norm=pre_norm, activation=act, res_attention=res_attention,
                                           n_layers=n_layers,
                                           store_attn=store_attn)
        self.encoder3 = TransformerEncoder(q_len / 4, d_model * 4, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                           attn_dropout=attn_dropout, dropout=dropout,
                                           pre_norm=pre_norm, activation=act, res_attention=res_attention,
                                           n_layers=n_layers,
                                           store_attn=store_attn)
    ...
    def forward(self, x) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]
        ...
        u = torch.reshape(u, (-1, n_vars, u.shape[-2], u.shape[-1]))  # u: [bs * nvars x patch_num / 4 x d_model * 4]
        u = u.permute(0, 1, 3, 2)  # u: [bs * nvars x d_model * 4 x patch_num / 4]
...
class TransformerEncoder(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None,
        ...
        self.layers = nn.ModuleList(
            [TransformerEncoderLayer(q_len, d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                     attn_dropout=attn_dropout, dropout=dropout,
